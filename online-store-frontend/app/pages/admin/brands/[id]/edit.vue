...
const { data } = await useFetch<BrandDto[]>(`${apiBase}/brands`, {
...
const match = computed(() => data.value.find((b) => b.id === rawId) ?? null);
...
async function onSubmit() {
  ...
  loading.value = true;
  try {
    await $fetch(`${apiBase}/brands/${rawId}`, {
      method: "PUT",
      headers: { Authorization: `Bearer ${auth.token}` },
      body: parsed.data,
    });
    ...
    toast.error(e?.data?.message ?? "Could not update brand.");
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div class="max-w-md space-y-6">
    ...
    <div v-if="isNotFound" class="rounded-xl border border-gray-200 p-8 text-center">
      <p class="text-gray-700">Brand not found.</p>
      <div class="mt-4 flex justify-center">
        <NuxtLink to="/admin/brands">
          <BaseButton variant="secondary">Back to brands</BaseButton>
        </NuxtLink>
      </div>
    </div>
